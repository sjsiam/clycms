<?php
// Get current URI path
$currentPath = $_SERVER['REQUEST_URI'];
?>

<div class="sidebar">
    <div class="p-4">
        <h4 class="text-center mb-4">
            <i class="fas fa-cogs"></i> CMS Admin
        </h4>
    </div>

    <nav class="nav flex-column">
        <a class="nav-link <?= ($currentPath == '/admin') ? 'active' : '' ?>" href="/admin">
            <i class="fas fa-dashboard me-2"></i> Dashboard
        </a>
        <a class="nav-link <?= ($currentPath == '/admin/posts') ? 'active' : '' ?>" href="/admin/posts">
            <i class="fas fa-file-alt me-2"></i> Posts
        </a>
        <a class="nav-link <?= ($currentPath == '/admin/pages') ? 'active' : '' ?>" href="/admin/pages">
            <i class="fas fa-file me-2"></i> Pages
        </a>
        <a class="nav-link <?= ($currentPath == '/admin/media') ? 'active' : '' ?>" href="/admin/media">
            <i class="fas fa-images me-2"></i> Media
        </a>
        <a class="nav-link <?= ($currentPath == '/admin/users') ? 'active' : '' ?>" href="/admin/users">
            <i class="fas fa-users me-2"></i> Users
        </a>
        <a class="nav-link <?= ($currentPath == '/admin/themes') ? 'active' : '' ?>" href="/admin/themes">
            <i class="fas fa-palette me-2"></i> Themes
        </a>
        <a class="nav-link <?= ($currentPath == '/admin/plugins') ? 'active' : '' ?>" href="/admin/plugins">
            <i class="fas fa-plug me-2"></i> Plugins
        </a>
        <a class="nav-link <?= ($currentPath == '/admin/settings') ? 'active' : '' ?>" href="/admin/settings">
            <i class="fas fa-cog me-2"></i> Settings
        </a>

        <hr class="mx-3 my-3" style="border-color: #34495e;">

        <a class="nav-link" href="/" target="_blank">
            <i class="fas fa-external-link-alt me-2"></i> View Site
        </a>
    </nav>
</div>